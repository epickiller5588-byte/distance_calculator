...
let currentLang='th';
let map,directionsService,trafficLayer,placesService,markerA=null,markerB=null,currentPos=null,lastRoutes=[],selectedRouteIndex=0,polyLines=[],poiMarkers=[];
...
acEnd.addListener('place_changed',()=>{const place=acEnd.getPlace();if(place?.geometry?.location){const loc=place.geometry.location;if(!phuketBounds.contains(loc)){alert(i18n[currentLang].onlyPhuket);return;}if(!markerB)markerB=new google.maps.Marker({map,icon:{path:google.maps.SymbolPath.CIRCLE,scale:8,fillColor:'#f44336',fillOpacity:1,strokeWeight:0}});markerB.setPosition(loc);markerB.setMap(map);map.panTo(loc);if(markerA&&markerA.getPosition())computeRoutes(markerA.getPosition(),loc);}});

document.getElementById("btn-current").addEventListener('click',()=>{if(navigator.geolocation){navigator.geolocation.getCurrentPosition(pos=>{const loc={lat:pos.coords.latitude,lng:pos.coords.longitude};if(!phuketBounds.contains(new google.maps.LatLng(loc.lat,loc.lng))){alert(i18n[currentLang].outsidePhuket);return;}if(!markerA)markerA=new google.maps.Marker({map,icon:{path:google.maps.SymbolPath.CIRCLE,scale:8,fillColor:'#1e88e5',fillOpacity:1,strokeWeight:0}});markerA.setPosition(loc);markerA.setMap(map);map.panTo(loc);currentPos=loc;if(markerB&&markerB.getPosition())computeRoutes(currentPos,markerB.getPosition());});}});
...
document.getElementById("btn-reset").addEventListener('click',()=>{markerA?.setMap(null);markerB?.setMap(null);markerA=markerB=null;lastRoutes=[];document.getElementById("routesList").innerHTML=i18n[currentLang].noRoute;polyLines.forEach(p=>p.setMap(null));polyLines=[];});
...
directionsService.route({origin,destination:dest,travelMode:'DRIVING',provideRouteAlternatives:true},(res,status)=>{if(status==='OK'){lastRoutes=res.routes;renderRoutes();}else{alert(status);}});
}
...
const div=document.createElement('div');div.className='route-card';div.tabIndex=0;div.addEventListener('click',()=>{selectedRouteIndex=i;highlightRoute();});const leg=r.legs[0];const distKm=km(leg.distance.value);const durMin=Math.round(leg.duration.value/60);const vkey=document.getElementById("vehicle").value||'grabCar';const fare=calculateFare(r,vkey);
div.innerHTML=`<div class="route-left"><div class="route-title">${i+1}. ${leg.start_address.split(',')[0]} → ${leg.end_address.split(',')[0]}</div><div class="route-meta">${distKm} ${i18n[currentLang].kmLabel}, ${durMin} ${i18n[currentLang].minsLabel}</div></div><div class="fare-pill">${fare} ฿</div>`;
list.appendChild(div);

const poly=new google.maps.Polyline({path:r.overview_path,strokeColor:'#1e88e5',strokeOpacity:0.7,strokeWeight:5,map:map});
polyLines.push(poly);
});
highlightRoute();
}
...
polyLines.forEach((p,i)=>p.setOptions({strokeOpacity:i===selectedRouteIndex?1:0.4,strokeWeight:i===selectedRouteIndex?6:4}));
const cards=document.querySelectorAll('.route-card');cards.forEach((c,i)=>{c.classList.toggle('selected',i===selectedRouteIndex);});
}
</script>
