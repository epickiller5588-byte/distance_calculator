...
<script>
/* --- Data & i18n (เหมือนเวอร์ชันก่อนหน้า) --- */
const vehicleRates={grabCar:{base:35,perKm:10,freeKm:2,min:50},grabBike:{base:20,perKm:7,freeKm:1,min:25},boltEconomy:{base:50,perKm:10,freeKm:2,min:100},boltStandard:{base:100,perKm:12,freeKm:2,min:200},boltVan:{base:250,perKm:15,freeKm:2,min:300},boltXL:{base:200,perKm:15,freeKm:2,min:300},boltTaxi:{base:100,perKm:12,freeKm:2,min:200},inDrive:{base:30,perKm:9,freeKm:1,min:80},taxiMeter:{base:50,perKm:12,freeKm:2,min:100},smartBus:{base:100,perKm:0,freeKm:0,min:100,fixed:true},songthaew:{base:40,perKm:0,freeKm:0,min:40,fixed:true},pinkBus:{base:15,perKm:0,freeKm:0,min:15,fixed:true},tukTuk:{base:200,perKm:0,freeKm:0,min:200,fixed:true},motoTaxi:{base:80,perKm:0,freeKm:0,min:80,fixed:true}};
...
function initMap(){
  ...
  acEnd.addListener('place_changed',()=>{const place=acEnd.getPlace();if(place?.geometry?.location){const loc=place.geometry.location;if(!phuketBounds.contains(loc)){alert(i18n[currentLang].outsidePhuket);return;} if(!markerB) markerB=new google.maps.Marker({map,icon:'http://maps.google.com/mapfiles/ms/icons/red-dot.png',draggable:true}); markerB.addListener('dragend',()=>{if(markerA && markerB) computeRoutes(markerA.getPosition(),markerB.getPosition());}); markerB.setMap(map); markerB.setPosition(loc); map.panTo(loc); if(markerA && markerB) computeRoutes(markerA.getPosition(),markerB.getPosition());}});

  document.getElementById('btn-current').addEventListener('click',()=>{navigator.geolocation.getCurrentPosition(pos=>{const loc={lat:pos.coords.latitude,lng:pos.coords.longitude}; if(!phuketBounds.contains(new google.maps.LatLng(loc.lat,loc.lng))){alert(i18n[currentLang].onlyPhuket);return;} if(!markerA) markerA=new google.maps.Marker({map,icon:{path:google.maps.SymbolPath.CIRCLE,scale:8,fillColor:'#1e88e5',fillOpacity:1,strokeWeight:0}}); markerA.setPosition(loc); markerA.setMap(map); map.panTo(loc); currentPos=loc; if(markerB && markerB.getPosition()) computeRoutes(currentPos, markerB.getPosition());});});
  ...
  document.getElementById('btn-reset').addEventListener('click',()=>{if(markerA){markerA.setMap(null);markerA=null;}if(markerB){markerB.setMap(null);markerB=null;}lastRoutes=[];selectedRouteIndex=0;document.getElementById('routesList').innerHTML=i18n[currentLang].noRoute;});
  ...
  document.getElementById('vehicle').addEventListener('change',()=>{renderRoutes(lastRoutes);});

  // คลิกบน map เพื่อปัก markerB หลังเลือก markerA
  map.addListener('click',(e)=>{if(!markerA) return; const loc=e.latLng; if(!phuketBounds.contains(loc)){alert(i18n[currentLang].outsidePhuket);return;} if(!markerB){markerB=new google.maps.Marker({position:loc,map,icon:'http://maps.google.com/mapfiles/ms/icons/red-dot.png',draggable:true}); markerB.addListener('dragend',()=>{if(markerA && markerB) computeRoutes(markerA.getPosition(),markerB.getPosition());});}else{markerB.setPosition(loc);} map.panTo(loc); if(markerA && markerB) computeRoutes(markerA.getPosition(),markerB.getPosition()); });
}

function computeRoutes(origin,dest){
  const request={origin,destination:dest,travelMode:'DRIVING',provideRouteAlternatives:true};
  directionsService.route(request,(res,status)=>{if(status==='OK'){lastRoutes=res.routes;selectedRouteIndex=0;renderRoutes(res.routes);}});
}

function renderRoutes(routes){
  const container=document.getElementById('routesList'); container.innerHTML=''; polyLines.forEach(pl=>pl.setMap(null)); polyLines=[];
  const vehicleKey=document.getElementById('vehicle').value||'grabCar';
  routes.forEach((r,i)=>{const div=document.createElement('div'); div.className='route-card'+(i===selectedRouteIndex?' selected':''); div.addEventListener('click',()=>{selectedRouteIndex=i;renderRoutes(routes);}); const dist=km(r.legs[0].distance.value); const dur=Math.round(r.legs[0].duration.value/60); const fare=calculateFare(r,vehicleKey); div.innerHTML=`<div class="route-left"><div class="route-title">${i18n[currentLang].routeLabel} ${i+1}</div><div class="route-meta">${dist} ${i18n[currentLang].kmLabel}, ${dur} ${i18n[currentLang].minsLabel}</div></div><div class="fare-pill">${fare} ฿</div>`; container.appendChild(div); const pl=new google.maps.Polyline({path:r.overview_path,map:map,strokeColor:i===selectedRouteIndex?'#1e88e5':'#a0c4ff',strokeOpacity:1,strokeWeight:i===selectedRouteIndex?5:3}); polyLines.push(pl);});
}
